...
| `[ERROR] A rebase with unresolved conflicts is in progress on …` (or `merge`, `revert`, `An am`) | another git operation is half-finished, and no stage runs over one: `to-staging`, `resolved` and `deployable` each stop and name it | the refusal names both routes: `git rebase --continue` (or `merge` / `revert` / `am`) finishes it, `git rebase --abort` drops it; then re-run the stage. While files are still conflicted it says to fix them and `git add` them first, which is what `--continue` waits for |
